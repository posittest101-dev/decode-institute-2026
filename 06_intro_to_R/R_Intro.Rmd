...
How do we calculate the average vo_max1 for male and female control rats?

```{r, solution=TRUE}

```

Create a new table containing only female rats whose vo_max2 is above the overall average.

```{r, solution=TRUE}

```

How would we create a new data object with only the PLASMA tissue samples?

```{r, solution=TRUE}

```

Calculate the percent improvement in vo2 max from session 1 to session 2. What is the pid of the rat that improved the most?
...
Now let's look at the overall distribution of the first VO2 max measure. We can use the `ggplot` function on the `pheno_dist` data object, plotting the `vo_max1` on the `x`-axis as a `histogram`. Let's do this in the code block below:
...
We can also look at the distribution of several groups using a box plot. Let's create a `geom_boxplot` looking at the `sex` on the `x`-axis and `vo_max1` on the `y`-axis in the code block below:
...
Rather than creating a single graph with the variables in differing colors, we can instead use the `facet_wrap` function to create separate graphs for each value in the `sex` variable. The `facet_wrap` function will automatically divide a plot into subplots based on the values of one or more discrete variables. Plots can be organized into variables by rows, columns, or wrapped into a rectangular layout. ![](images/faceting.png)

Let's create a `geom_point` plot using the `pheno_dist` data object with `vo_max1` in the `x`-axis and `vo_max2` in the `y`-axis. Add a `geom_smooth` line using the `lm` `method`. Now let's use the `facet_wrap` function to create subplots for each `sex` using the `vars` option in the empty code chunk below:
...
```{r, solution=TRUE}
#Note this is one I will walk through with the group
